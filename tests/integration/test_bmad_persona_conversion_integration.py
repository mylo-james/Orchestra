...
class TestBmadPersonaConversionIntegration:
    ...
    def test_convert_specific_bmad_personas(self, inventory, converter):
        ...
        for name, persona in test_personas.items():
            ...
            assert result.success is True, f"Failed to convert {name}: {result.validation_errors}"
            assert result.persona_spec is not None
            assert result.yaml_content is not None
    ...
    def test_yaml_output_quality(self, inventory, converter):
        ...
        required_identity_fields = ["name", "id", "title", "role", "icon", "when_to_use", "style", "focus"]
        for field in required_identity_fields:
            assert field in identity, f"Missing identity field: {field}"
            assert identity[field] is not None and identity[field] != "", f"Empty identity field: {field}"
        ...
        required_dep_fields = ["knowledge_sources", "tasks", "tools", "templates", "required_services"]
        for field in required_dep_fields:
            assert field in resource_deps, f"Missing resource dependency field: {field}"
            assert isinstance(resource_deps[field], list), f"Resource dependency {field} should be a list"
    ...
    def test_metadata_extraction_from_real_files(self, inventory, converter):
        ...
        for result in successful_results:
            ...
            if (len(spec.behavioral_contract.core_principles) > 0 or
                len(spec.resource_dependencies.tools) > 0 or
                len(spec.command_interface.commands) > 1):  # More than just help command
        ...
        assert personas_with_rich_data >= 1, "Should extract rich metadata from at least 1 persona"

    def test_backward_compatibility_validation(self, inventory, converter, persona_loader):
        ...
        bmad_personas = inventory.scan_agents()
        target_personas = converter.identify_target_personas(bmad_personas)[:5]  # Test with 5
        ...
        for persona_id, original_spec in existing_specs.items():
            ...
            assert current_spec is not None, f"Existing persona {persona_id} no longer loads"
    ...
    def test_schema_validation_with_real_data(self, inventory, converter):
        ...
        for result in results:
            assert result.success is True, f"Validation failed: {result.validation_errors}"
            
            # Double-check with explicit validation
            validation_result = converter.validate_persona_schema(result.persona_spec)
            assert validation_result.is_valid is True, f"Schema validation failed: {validation_result.errors}"
    ...
    def test_conversion_preserves_essential_information(self, inventory, converter):
        ...
        test_cases = {
            "dev.md": {
                "expected_role_keywords": ["developer", "development", "code", "implementation"],
        ...
        for persona in bmad_personas:
            if persona.name in test_cases:
                ...
                found_keywords = [kw for kw in test_case["expected_role_keywords"] 
                                if kw in combined_text]
                assert len(found_keywords) > 0, f"No expected keywords found in {persona.name} role/title"
                
                # Check commands if expected
                if test_case["should_have_commands"]:
                    assert len(spec.command_interface.commands) >= 1, f"No commands found for {persona.name}"
