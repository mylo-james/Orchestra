...
class TestPersonaDiscovery:
    ...
    def test_discover_all_personas_including_converted(
        self, runner, mock_persona_loader
    ):
        """Test discovering all personas including converted BMad personas (AC: 1)."""
        with patch(
            "orchestra.system.loader.PersonaLoader", return_value=mock_persona_loader
        ):
    ...
    def test_persona_details_display(self, runner, mock_persona_loader):
        ...
        assert (
            "TDD approach" in result.stdout
            or "clean maintainable code" in result.stdout
        )
    ...
    def test_filter_personas_by_category(self, runner, mock_persona_loader):
        ...
        with patch(
            ...
            result = runner.invoke(
                app, ["agent", "list-personas", "--category", "development"]
            )
...
class TestPersonaActivation:
    ...
    def test_activate_persona_command(self, runner, mock_persona_spec):
        ...
        with patch("orchestra.cli.commands.PersonaLoader") as mock_loader_class:
            ...
            assert result.exit_code == 0
            assert (
                "Activated persona: dev" in result.stdout
                or "Full Stack Developer" in result.stdout
            )
...
class TestPersonaCommandExecution:
    ...
    def test_list_persona_commands(self, runner, mock_persona_with_commands):
        ...
        with patch("orchestra.system.loader.PersonaLoader") as mock_loader_class:
            ...
            assert (
                "implement-feature" in result.stdout
                or "Implement a specific" in result.stdout
            )
    ...
    def test_execute_persona_command(self, runner, mock_persona_with_commands):
        ...
        with patch("orchestra.system.loader.PersonaLoader") as mock_loader_class:
            ...
            with patch("orchestra.system.agent.UniversalAgent") as mock_agent_class:
                ...
                result = runner.invoke(
                    app, ["agent", "exec", "implement", "--feature", "user-auth"]
                )
...
class TestEndToEndValidation:
    ...
    def mock_persona_with_resources(self):
        ...
        return PersonaSpec(
            ...
            resource_dependencies=ResourceDependencies(
                tasks=["review-tdd-implementation"], templates=["test-spec-tmpl"]
            ),
        )
    ...
    def test_end_to_end_task_execution(self, runner, mock_persona_with_resources):
        ...
        with patch("orchestra.system.loader.PersonaLoader") as mock_loader_class:
            ...
            with patch(
                "orchestra.system.resource_loader.ResourceLoader"
            ) as mock_resource_loader:
                with patch(
                    "orchestra.system.task_engine.TaskEngine"
                ) as mock_task_engine:
                    ...
                    mock_resource_loader.return_value.load_resource.return_value = (
                        mock_resource_result
                    )
                    mock_task_engine.return_value.execute_task.return_value = (
                        mock_execution_result
                    )

                    result = runner.invoke(
                        app, ["agent", "run-task", "review-tdd-implementation"]
                    )
    ...
    def test_end_to_end_template_rendering(self, runner, mock_persona_with_resources):
        ...
        with patch("orchestra.system.loader.PersonaLoader") as mock_loader_class:
            ...
            with patch(
                ...
                with patch(
                    "orchestra.system.template_processor.TemplateProcessor"
                ) as mock_template_processor:
                    mock_resource_result = Mock()
                    mock_resource_result.success = True
                    mock_resource_result.metadata = Mock()
                    ...
                    mock_template_processor.return_value.render_template.return_value = (
                        mock_render_result
                    )
    ...
    def test_end_to_end_checklist_execution(self, runner, mock_persona_with_resources):
        ...
        with patch("orchestra.system.loader.PersonaLoader") as mock_loader_class:
            ...
            with patch(
                ...
                with patch(
                    "orchestra.system.checklist_engine.ChecklistEngine"
                ) as mock_checklist_engine:
                    mock_resource_result = Mock()
                    mock_resource_result.success = True
                    mock_resource_result.metadata = Mock()
                    mock_resource_result.content = (
                        "# TDD DoD\n- [x] Tests written\n- [ ] Code implemented"
                    )
                    ...
                    mock_checklist_engine.return_value.execute_checklist.return_value = (
                        mock_execution_result
                    )

                    result = runner.invoke(
                        app, ["agent", "check", "tdd-dev-dod-checklist"]
                    )
...
class TestAuditAndSecurity:
    ...
    def test_persona_activation_audit_logging(self, runner):
        ...
        with patch("orchestra.system.loader.PersonaLoader") as mock_loader_class:
            ...
            with patch("orchestra.utils.logging.get_logger") as mock_logger:
                ...
                call_args = [
                    call[0] for call in mock_logger_instance.info.call_args_list
                ]
                audit_logged = any(
                    "persona" in str(args).lower() and "activate" in str(args).lower()
    ...
    def test_command_execution_audit_logging(self, runner):
        ...
        with patch("orchestra.system.loader.PersonaLoader") as mock_loader_class:
            mock_loader = Mock()
            mock_persona = Mock()
            mock_persona.command_interface.commands = {
            ...
            with patch("orchestra.utils.logging.get_logger") as mock_logger:
                ...
                with patch("orchestra.system.agent.UniversalAgent") as mock_agent:
                    mock_agent.return_value.execute_command.return_value = {
                        "status": "success"
                    }
    ...
    def test_security_monitoring_integration(self, runner):
        ...
        with patch(
            "orchestra.security.ai_agent_monitor.AIAgentMonitor"
        ) as mock_monitor:
...
class TestIntegrationWithRealData:
    ...
    def test_real_persona_activation(self, runner):
        ...
        assert (
            "Activated persona: Alex" in result.stdout
            or "✅ Activated persona:" in result.stdout
        )
