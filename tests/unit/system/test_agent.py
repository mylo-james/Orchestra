...
import src.system.agent

from src.system.agent import UniversalAgent
from src.system.specs import PersonaSpec, PersonaIdentity, BehavioralContract, CommandInterface, ResourceDependencies
...
class TestUniversalAgent:
    ...
    def mock_persona_spec(self):
        ...
        spec.behavioral_contract.core_principles = ["Test Principle 1", "Test Principle 2"]
        spec.behavioral_contract.halt_conditions = ["Security validation failures", "Ambiguous requirements"]
        ...
        spec.command_interface.commands = {
            "test_command": Mock(),
            "analyze": Mock()
        }
        
        spec.resource_dependencies = Mock(spec=ResourceDependencies)
        spec.resource_dependencies.tools = ["github-tools"]  # This list is already iterable
    ...
    def test_initialization_fails_with_invalid_persona(self):
        ...
        with patch('src.system.agent.PersonaLoader', return_value=mock_loader):
            with pytest.raises(ValueError, match="Failed to load persona: invalid-persona"):
    ...
    def test_configure_from_persona(self, universal_agent, mock_persona_spec):
        ...
        assert universal_agent.core_principles == mock_persona_spec.behavioral_contract.core_principles
        assert universal_agent.halt_conditions == mock_persona_spec.behavioral_contract.halt_conditions
        assert universal_agent.interaction_style == mock_persona_spec.behavioral_contract.interaction_style
        assert universal_agent.execution_model == mock_persona_spec.command_interface.execution_model
    ...
    async def test_execute_command_success(self, universal_agent, mock_persona_spec):
        ...
        mock_command.execution_pattern = "analyze → execute → report"  # Make it a splittable string
    ...
    @pytest.mark.asyncio
    async def test_execute_command_with_confirmation(self, universal_agent, mock_persona_spec):
        """Test command execution with confirmation required."""
        mock_command = Mock()
        mock_command.requires_confirmation = True
        mock_command.timeout_seconds = 30
        mock_command.execution_pattern = "validate → confirm → execute → report"  # Make it a splittable string
        mock_persona_spec.get_command.return_value = mock_command
    ...
    async def test_execute_command_timeout(self, universal_agent, mock_persona_spec):
        ...
        with patch.object(universal_agent, '_execute_pattern', side_effect=asyncio.TimeoutError()):
            result = await universal_agent.execute_command("test_command")
    ...
    async def test_execute_command_exception(self, universal_agent, mock_persona_spec):
        ...
        with patch.object(universal_agent, '_execute_pattern', side_effect=Exception("Test error")):
            result = await universal_agent.execute_command("test_command")
    ...
    async def test_execute_step_implement(self, universal_agent):
        ...
        result = await universal_agent._execute_step("implement", {"requirements": "test"})
