...
class TestConflictDetection:
    ...
    @pytest.mark.asyncio
    async def test_detect_conflict_no_conflict_different_content(
    ...
    @pytest.mark.asyncio
    async def test_detect_conflict_high_similarity_concurrent(
        self, service, sample_chunks
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_conflict_generates_embeddings_if_missing(
        self, service, sample_chunks
    ):
...
class TestConflictResolution:
    ...
    @pytest.mark.asyncio
    async def test_resolve_conflict_auto_strategy_selection(
        self, service, sample_conflict
    ):
    ...
    @pytest.mark.asyncio
    async def test_merge_vote_strategy_higher_confidence_wins(
        self, service, sample_conflict
    ):
    ...
    @pytest.mark.asyncio
    async def test_merge_hybrid_strategy_with_escalation(
        self, service, sample_conflict
    ):
        ...
        with (
            patch.object(service, "_needs_human_escalation", return_value=True),
            patch.object(
                service, "_merge_conservative", return_value=Mock()
...
class TestHumanEscalation:
    ...
    async def test_resolve_escalation_success(self, service, sample_conflict):
        ...
        resolved_chunk = await service.resolve_escalation(
            "conflict-1", "Human-resolved content"
        )
...
class TestMergeStrategies:
    ...
    @pytest.mark.asyncio
    async def test_merge_hybrid_strategy_intelligent_merge(
        self, service, complex_conflict
    ):
    ...
    @pytest.mark.asyncio
    async def test_merge_vote_strategy_lower_confidence_wins(
        self, service, complex_conflict
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_conflict_with_explicit_strategies(
        self, service, complex_conflict
    ):
        """Test conflict resolution with explicitly specified strategies."""
        # Test CONSERVATIVE strategy
        result = await service.resolve_conflict(
            complex_conflict, MergeStrategy.CONSERVATIVE
        )
        assert result.content == complex_conflict.version_a.content

        # Test AGGRESSIVE strategy
        result = await service.resolve_conflict(
            complex_conflict, MergeStrategy.AGGRESSIVE
        )
...
class TestUtilityMethods:
    ...
    def test_classify_severity_edge_cases(self, service):
        ...
        assert (
            service._classify_severity(0.849) == SeverityLevel.CRITICAL
        )  # Below all thresholds -> CRITICAL
        assert (
            service._classify_severity(0.851) == SeverityLevel.LOW
        )  # Within LOW threshold
        assert (
            service._classify_severity(0.999) == SeverityLevel.CRITICAL
        )  # Very high similarity
        assert (
            service._classify_severity(1.0) == SeverityLevel.CRITICAL
        )  # Perfect similarity

        # Test edge of ranges
        assert (
            service._classify_severity(0.85) == SeverityLevel.LOW
        )  # LOW threshold start
        assert (
            service._classify_severity(0.90) == SeverityLevel.MEDIUM
        )  # MEDIUM threshold start
        assert (
            service._classify_severity(0.95) == SeverityLevel.HIGH
        )  # HIGH threshold start
        assert (
            service._classify_severity(0.98) == SeverityLevel.CRITICAL
        )  # CRITICAL threshold start
...
class TestMissingCoverageLines:
    ...
    async def test_resolve_conflict_vote_strategy_execution(self, service):
        ...
        assert result is not None
        assert (
            result.content == "Updated authentication with OAuth"
