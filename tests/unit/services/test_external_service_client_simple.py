...
class TestExternalServiceClient:
    ...
    def test_generate_code_with_openai(self, mock_protect, mock_httpx, mock_openai, mock_settings):
    ...
    async def test_create_github_pr(self, mock_protect, mock_httpx, mock_openai, mock_settings):
        """Test GitHub PR creation."""
        mock_protect.side_effect = lambda service, fallback_result: lambda func: func
        
        # Mock HTTP response
        mock_response = Mock()
        mock_response.json.return_value = {"id": 123, "url": "https://github.com/test/pull/123"}
    ...
    @patch('src.services.external_service_client.httpx.AsyncClient')
    @patch('src.services.external_service_client.protect_external_service_async')
    async def test_query_qdrant_knowledge(self, mock_protect, mock_httpx, mock_openai, mock_settings):
    ...
    @patch('src.services.external_service_client.httpx.AsyncClient')
    @patch('src.services.external_service_client.protect_external_service_async')
    async def test_start_temporal_workflow(self, mock_protect, mock_httpx, mock_openai, mock_settings):
        """Test Temporal workflow start."""
        mock_protect.side_effect = lambda service, fallback_result: lambda func: func
        
        client = ExternalServiceClient(mock_settings)
        result = await client.start_temporal_workflow("test_workflow", {"param": "value"})
...
class TestSecureAIAgent:
    ...
    @pytest.mark.asyncio
    @patch('src.services.external_service_client.ExternalServiceClient')
    async def test_generate_and_commit_code_success(self, mock_client_class, mock_settings):
        ...
        mock_client.generate_code_with_openai_async = AsyncMock(return_value="def test(): pass")
        mock_client.create_github_pr = AsyncMock(return_value={"id": 123})
        mock_client.start_temporal_workflow = AsyncMock(return_value={"workflow_id": "wf123"})
    ...
    @pytest.mark.asyncio
    @patch('src.services.external_service_client.ExternalServiceClient')
    async def test_generate_and_commit_code_with_fallbacks(self, mock_client_class, mock_settings):
        ...
        mock_client.start_temporal_workflow = AsyncMock(
            side_effect=CircuitBreakerError("temporal", CircuitState.OPEN, "Temporal down")
        )
        mock_client_class.return_value = mock_client
    ...
    @pytest.mark.asyncio
    @patch('src.services.external_service_client.ExternalServiceClient')
    async def test_generate_and_commit_code_with_exception(self, mock_client_class, mock_settings):
...
class TestCircuitBreakerIntegration:
    ...
    async def test_circuit_breaker_test_function(
        self, mock_failing, mock_stats, mock_github, mock_qdrant, 
        mock_temporal, mock_openai
    ):
