...
class TestExecuteAgentActivity:
    ...
    @pytest.mark.asyncio
    @patch('src.workflows.activities.get_registry')
    async def test_execute_agent_activity_with_universal_agent(self, mock_get_registry):
        ...
        mock_agent.execute_command = AsyncMock(return_value={
            "success": True,
        ...
        mock_agent.execute_command.assert_called_once_with("plan", {
    ...
    @pytest.mark.asyncio
    @patch('src.workflows.activities.get_registry')
    async def test_execute_agent_activity_with_non_universal_agent(self, mock_get_registry):
    ...
    async def test_execute_agent_activity_persona_mapping(self, mock_get_registry):
        ...
        params = {
            ...
            "operation": "implement",
            "context": {"session_id": "test", "correlation_id": "test", "working_memory": {}}
        }
    ...
    async def test_execute_agent_activity_unknown_operation(self, mock_get_registry):
        ...
        mock_agent.execute_command.assert_called_once_with("unknown_operation", {
            "context": params["context"],
            "working_memory": {},
            "request": ""
        })
...
class TestUniversalAgentExecution:
    ...
    async def test_execute_with_universal_agent_success(self):
        ...
        mock_agent.execute_command.assert_called_once_with("implement-story", {
            "context": context,
    ...
    async def test_execute_with_universal_agent_unknown_operation(self):
        ...
        # Should use operation directly as command
        result = await _execute_with_universal_agent(mock_agent, "custom_command", context)
        
        assert result["conversation"]["role"] == "orchestrator"
        mock_agent.execute_command.assert_called_once_with("custom_command", {
            "context": context,
            "working_memory": {},
            "request": ""
        })
...
class TestValidateContextActivity:
    ...
    async def test_validate_context_activity_multiple_errors(self):
        ...
        assert result["valid"] is False
        assert len(result["errors"]) >= 4  # Missing session_id, correlation_id, security_context, plus invalid schema and task_state
    ...
    async def test_validate_context_activity_valid_task_states(self):
        ...
        valid_states = ["planning", "implementing", "reviewing", "releasing", "completed", "failed", None]
...
class TestCreateGitHubPRActivity:
    ...
    async def test_create_github_pr_activity_with_exception(self):
        ...
        with patch('src.workflows.activities.asyncio.sleep', side_effect=Exception("GitHub API Error")):
        ...
        def create_agent(persona_id):
            ...
            if persona_id == "orchestrator":
                agent.execute_command = AsyncMock(return_value={
                    "success": True,
                    "result": {"plan": {"steps": ["analyze", "design", "implement"]}}
                })
            elif persona_id == "dev":
                agent.execute_command = AsyncMock(return_value={
                    "success": True,
