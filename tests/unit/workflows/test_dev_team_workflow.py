...
async def test_orchestrator_grab_work_edit_upsert_operations():
    ...
    async def mock_orchestrator_execution(params):
        ...
        if operation == "plan":
            return {
                "conversation": {"role": "orchestrator", "content": "Planning analysis complete"},
                "confidence": 0.9,
                "memory_updates": {
                    "grab_results": {"patterns": ["auth_pattern", "api_pattern"]},
                    "work_analysis": {"complexity": "medium", "strategy": "incremental"},
        ...
        return {"conversation": {"role": "test", "content": "Test"}, "confidence": 0.8, "memory_updates": {}, "next_action": "continue"}
    
    # Test GRAB operation
    grab_result = await mock_orchestrator_execution({"operation": "plan", "agent_type": "orchestrator"})
    assert "grab_results" in grab_result["memory_updates"]
    assert grab_result["memory_updates"]["grab_results"]["patterns"] == ["auth_pattern", "api_pattern"]
    ...
    assert "auth_pattern_v2" in grab_result["memory_updates"]["edit_refinements"]["updated_patterns"]
    ...
    async def mock_kickstart_execution(context):
        return {
            ...
            "handoff_context": {
                ...
                "implementation_guidance": {"start_with": "models", "then": "endpoints"}
            }
        }
...
async def test_persona_aware_handoffs():
    ...
    for scenario in task_scenarios:
        ...
        assert selected_persona == scenario["expected_persona"], f"Failed for {scenario['task_type']}"
...
async def test_dynamic_persona_selection():
    ...
    context_scenarios = [
        {
            "context": {
                "code_base_analysis": {"primary_language": "python", "framework": "fastapi"},
                "security_requirements": {"level": "high", "compliance": ["SOC2", "PCI"]},
        ...
        {
            "context": {
                "code_base_analysis": {"primary_language": "typescript", "framework": "react"},
    ...
    def analyze_context_for_persona(context):
        ...
        if context.get("security_requirements", {}).get("level") == "high":
            return "security-focused-dev"
        elif "ui_requirements" in context or "react" in context.get("code_base_analysis", {}).get("framework", ""):
            return "frontend-specialist"
        elif "services" in context.get("code_base_analysis", {}) or "performance_requirements" in context:
            return "backend-dev"
        else:
            return "dev"
...
async def test_receive_handbacks_from_specialized_agents():
    ...
    handback_scenarios = [
        {
            ...
            "handback": {
                "implementation_details": {"auth_method": "JWT", "encryption": "AES-256"},
                "security_insights": {"vulnerabilities_addressed": 3, "compliance_met": ["SOC2"]},
                "lessons_learned": {"pattern": "security_first_design", "effectiveness": "high"}
            }
        },
        {
            "agent": "frontend-specialist", 
            "handback": {
                "implementation_details": {"components_created": 5, "accessibility_score": "AA"},
                "ui_insights": {"user_feedback": "positive", "performance_impact": "minimal"},
                "lessons_learned": {"pattern": "component_reusability", "effectiveness": "high"}
            }
        },
        {
            "agent": "release",
            "handback": {
                "release_details": {"pr_created": True, "tests_passed": True, "deployment_ready": True},
                "process_insights": {"automation_effectiveness": "high", "time_saved": "30min"},
                "lessons_learned": {"pattern": "automated_validation", "effectiveness": "high"}
            }
        }
    ]
    ...
    for scenario in handback_scenarios:
        result = await mock_orchestrator_receive_handback(scenario["agent"], scenario["handback"])
...
async def test_cross_agent_analysis_and_knowledge_synthesis():
    ...
    async def mock_cross_agent_analysis(insights_data):
        ...
        avg_quality = sum(data.get("decision_quality", data.get("code_quality", data.get("deployment_quality", 0))) 
                         for data in insights_data.values()) / total_agents
        total_time = sum(data.get("time_to_plan", data.get("time_to_implement", data.get("time_to_release", 0)))
        ...
        return {
            ...
            "optimization_opportunities": ["reduce_handoff_time", "pattern_standardization"]
        }
    
    analysis_result = await mock_cross_agent_analysis(agent_insights)
    
    assert "cross_agent_patterns" in analysis_result
    assert len(analysis_result["cross_agent_patterns"]) >= 6  # Should find patterns from all agents
...
async def test_knowledge_synthesis_and_quality_control():
    ...
    knowledge_updates = [
        ...
        },
        {
            ...
            "data": {"jwt_implementation": {"security_score": 0.98, "performance": "good"}},
            "confidence": 0.92
        },
        {
            "source": "release",
            "pattern_type": "deployment",
            "data": {"security_deployment": {"automation_level": "high", "reliability": 0.96}},
            "confidence": 0.88
        }
    ]
    ...
    async def mock_knowledge_synthesis(updates):
        ...
        for update in updates:
            ...
            if update["confidence"] < 0.9:
                quality_issues.append(f"Low confidence from {update['source']}: {update['confidence']}")
...
async def test_full_workflow_integration_all_phases():
    ...
    async def mock_full_workflow_execution():
        ...
        phases = ["security_validation", "orchestrator_planning", "developer_implementation", "release_operations", "audit_completion"]
        ...
        return {
            ...
            "context_integrity": all(log["context_preserved"] for log in workflow_execution_log),
...
class TestDevTeamWorkflowComprehensive:
    ...
    async def test_orchestrator_planning_execution(self):
        ...
        with patch('temporalio.workflow.execute_activity') as mock_activity:
            ...
            mock_activity.return_value = {
                "conversation": {"role": "orchestrator", "content": "Planning completed"},
                "confidence": 0.9,
            ...
            workflow = DevTeamWorkflow()
            workflow.context = WorkflowContext(
                security_context=SecurityContext(user_id="test", permissions=["read", "write"]),
    ...
    async def test_developer_implementation_execution(self):
        ...
        with patch('temporalio.workflow.execute_activity') as mock_activity:
            ...
            mock_activity.return_value = {
                "conversation": {"role": "developer", "content": "Implementation completed"},
                "confidence": 0.95,
            ...
            workflow.context = WorkflowContext(
                security_context=SecurityContext(user_id="test", permissions=["read", "write"]),
                working_memory={"request": "Implement feature X", "planning_result": "planned"}
            )
    ...
    async def test_release_operations_execution(self):
        ...
        with patch('temporalio.workflow.execute_activity') as mock_activity:
            ...
            workflow.context = WorkflowContext(
                security_context=SecurityContext(user_id="test", permissions=["read", "write"]),
                working_memory={"request": "Deploy feature X", "implementation_result": "completed"}
            )
    ...
    async def test_audit_workflow_completion(self):
        ...
        with patch('temporalio.workflow.execute_activity') as mock_activity:
            mock_activity.return_value = {"success": True, "logged_at": "2024-01-01T00:00:00"}
    ...
    async def test_update_priority_signal(self):
        ...
        with patch.object(workflow, '_audit_workflow_signal') as mock_audit:
            ...
            # Verify audit called
            mock_audit.assert_called_once_with("priority_update", {"new_priority": "high"})
            ...
            def activity_side_effect(*args, **kwargs):
                ...
                if activity_name == "validate_security_activity":
                    ...
                    if activity_side_effect.call_count == 1:  # Orchestrator
                        return {"conversation": {"role": "orchestrator", "content": "Planning"}, "next_action": "implement"}
                    elif activity_side_effect.call_count == 2:  # Developer
                        return {"conversation": {"role": "developer", "content": "Implementing"}, "next_action": "release"}
                    else:  # Release
                        return {"conversation": {"role": "release", "content": "Releasing"}, "next_action": "complete"}
...
class TestDevTeamWorkflowEpic2PRDRequirements:
    ...
    async def test_grab_work_edit_upsert_operations(self):
        ...
        with patch('temporalio.workflow.execute_activity') as mock_activity:
            ...
            assert "planning_patterns" in workflow.context.working_memory["memory_updates"]
            assert "architectural_decisions" in workflow.context.working_memory["memory_updates"] 
            assert "implementation_strategy" in workflow.context.working_memory["memory_updates"]
            assert workflow.context.task_state == TaskState.IMPLEMENTING
        ...
        with patch('temporalio.workflow.execute_activity') as mock_activity:
            def persona_aware_activity(*args, **kwargs):
                ...
                if agent_type == 'developer':
                    ...
                    return {
                        "conversation": {"role": "developer", "content": "Selected security-focused-dev persona based on context"},
                        "confidence": 0.94,
                        "memory_updates": {"persona_selected": "security-focused-dev", "specialization": "security"},
                        "next_action": "release"
                    }
                return {"conversation": {"role": agent_type, "content": f"Persona-aware {agent_type}"}, "next_action": "continue"}
            ...
            assert workflow.context.current_agent == AgentType.DEVELOPER
            assert "persona_selected" in workflow.context.working_memory["memory_updates"]
            assert workflow.context.working_memory["memory_updates"]["persona_selected"] == "security-focused-dev"
    ...
    async def test_knowledge_synthesis_story_25(self):
        ...
        with patch('temporalio.workflow.execute_activity') as mock_activity:
            ...
            mock_activity.return_value = {
                "conversation": {"role": "release", "content": "Knowledge synthesis completed"},
                "confidence": 0.91,
                "memory_updates": {
...
async def test_workflow_completion_time_nfr2():
    ...
    assert result["phase_breakdown"]["developer_implementation"] <= 3600  # Most time in implementation
