...
class TestAIAgentValidator:
    ...
    def test_validate_operation_decorator_success(self, mock_monitor_class):
        ...
        mock_monitor_class.return_value = mock_monitor
        mock_monitor.check_input_security.return_value = {
            "is_safe": True, "violations": [], "action": "allow", "severity": "low"
        }
        mock_monitor.check_output_security.return_value = {
            "is_safe": True, "violations": [], "action": "allow", "severity": "low"
        }
    ...
    def test_validate_operation_decorator_input_blocked(self, mock_monitor_class):
        ...
        mock_monitor_class.return_value = mock_monitor
        mock_monitor.check_input_security.return_value = {
            "is_safe": False, "violations": ["prompt_injection"], "action": "block", "severity": "high"
        }
    ...
    def test_validate_operation_decorator_output_blocked(self, mock_monitor_class):
        ...
        mock_monitor.check_output_security.return_value = {
            "is_safe": False, "violations": ["malicious_code"], "action": "block", "severity": "critical"
        }
...
class TestAsyncValidatorMissingCoverage:
    ...
    async def test_validate_async_operation_decorator_success(self, validator):
        ...
        with patch.object(validator.monitor, 'log_agent_operation', return_value="async-op-123"), \
             patch.object(validator, '_validate_input', return_value=ValidationResult(True, [], "allow", "low")), \
             patch.object(validator, '_validate_output', return_value=ValidationResult(True, [], "allow", "low")):
    ...
    async def test_validate_async_operation_input_validation_failure(self, validator):
        ...
        with patch.object(validator, '_validate_input', return_value=ValidationResult(False, ["Malicious input"], "block", "high")):

            @validator.validate_async_operation("async_code_generation", block_on_violations=True)
    ...
    async def test_validate_async_operation_output_validation_failure(self, validator):
        ...
        with patch.object(validator.monitor, 'log_agent_operation', return_value="async-op-456"), \
             patch.object(validator, '_validate_input', return_value=ValidationResult(True, [], "allow", "low")), \
             patch.object(validator, '_validate_output', return_value=ValidationResult(False, ["Dangerous output"], "block", "high")):
    ...
    async def test_async_operation_with_logging_disabled(self, validator):
        ...
        with patch.object(validator.monitor, 'log_agent_operation') as mock_log, \
             patch.object(validator, '_validate_input', return_value=ValidationResult(True, [], "allow", "low")), \
             patch.object(validator, '_validate_output', return_value=ValidationResult(True, [], "allow", "low")):
