...
class ConversionStrategy:
    ...
    def convert_persona(self, bmad_persona: BmadContentItem) -> ResourceSchema:
        ...
        schema_definition = {
            "identity": {
                "name": bmad_persona.metadata.get(
                    "name", bmad_persona.name.replace(".md", "")
                ),
                "role": bmad_persona.metadata.get("role", "Agent"),
                "id": bmad_persona.name.replace(".md", "").lower(),
                "title": bmad_persona.metadata.get(
                    "title", bmad_persona.metadata.get("role", "Agent")
                ),
                "when_to_use": bmad_persona.metadata.get(
                    "when_to_use", "General purpose agent"
                ),
            ...
            "behavioral_contract": {
                ...
                "interaction_style": bmad_persona.metadata.get(
                    "interaction_style", "professional"
                ),
                "halt_conditions": bmad_persona.metadata.get("halt_conditions", []),
                "decision_framework": bmad_persona.metadata.get(
                    "decision_framework", "analytical"
                ),
            },
            "command_interface": {
                "execution_model": "sequential",
                "commands": self._convert_commands(
                    bmad_persona.metadata.get("commands", [])
                ),
    ...
    def convert_task(self, bmad_task: BmadContentItem) -> ResourceSchema:
        ...
        schema_definition = {
            "metadata": {
                ...
                "description": bmad_task.metadata.get(
            ...
            "execution": {
                ...
                "retry_policy": bmad_task.metadata.get(
    ...
    def convert_template(self, bmad_template: BmadContentItem) -> ResourceSchema:
        ...
        schema_definition = {
            "metadata": {
                "name": bmad_template.name.replace("-tmpl.yaml", "").replace(
                    "-tmpl.md", ""
                ),
                "version": bmad_template.version,
                "description": bmad_template.metadata.get(
    ...
    def convert_checklist(self, bmad_checklist: BmadContentItem) -> ResourceSchema:
        ...
        schema_definition = {
            "metadata": {
                ...
                "description": bmad_checklist.metadata.get(
            ...
            "validation": {
                "completion_threshold": bmad_checklist.metadata.get(
...
class BmadContentInventory:
    ...
    def _scan_directory(
        self, directory: Path, content_type: BmadContentType
    ...
    def _extract_metadata(
        self, file_path: Path, content_type: BmadContentType
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if yaml_match:
                try:
                    ...
                    logger.warning(
                        f"Error parsing YAML frontmatter in {file_path}: {e}"
                    )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "templates": [],
            "checklists": [],
            "summary": {"total_items": len(self.content_items), "by_type": {}},
        }
