...
class TestBmadPersonaConverter:
    ...
    def test_schema_validation_with_invalid_persona(self, converter):
        ...
        from orchestra.system.specs import PersonaIdentity
    ...
    def test_backward_compatibility_with_existing_personas(self, converter):
        ...
        try:
            ...
            for persona_id in existing_personas:
                ...
                assert (
                    spec is not None
                ), f"Failed to load existing persona: {persona_id}"
    ...
    def test_persona_load_performance(self, converter, inventory):
        ...
        for result in results:
            temp_file = Path(
                f"/tmp/test_persona_{result.persona_spec.identity.id}.yaml"
            )
        ...
        try:
            ...
            for temp_file in temp_files:
                ...
                temp_file.stem.replace("test_persona_", "")  # Parse but don't store
                try:
            ...
            assert (
                load_time_ms < 1000
            ), f"Load time {load_time_ms}ms exceeds performance requirement"
    ...
    def test_memory_footprint_requirement(self, converter, inventory):
        ...
        try:
            ...
            bmad_personas = inventory.scan_agents()
            target_personas = converter.identify_target_personas(bmad_personas)
            converter.convert_all_personas(target_personas)
            ...
            assert (
                memory_increase < 100
            ), f"Memory increase {memory_increase}MB is too high"
    ...
    def test_batch_conversion_with_output_directory(
        self, converter, inventory, tmp_path
    ):
...
class TestPersonaConversionResult:
    ...
    def test_successful_conversion_result(self):
        ...
        spec = PersonaSpec(
            identity=PersonaIdentity(
                name="Test", id="test", title="Test Persona", role="Test Role"
            )
        )
...
class TestPersonaValidationError:
    ...
    def test_validation_error_with_details(self):
        ...
        error = PersonaValidationError(
            "Validation failed", persona_id="test", field="name"
        )
