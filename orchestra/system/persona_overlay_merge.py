...
class OverlayMergeEngine:
    ...
    def _apply_overlay(
        self, persona: PersonaSpec, overlay: PersonaOverlay
    ...
    def _merge_section(
        ...
        if section_name == "behavioral_contract":
            ...
            return self._merge_command_interface(
                base_section, overlay_data, overlay_type
            )
        elif section_name == "resource_dependencies":
            return self._merge_resource_dependencies(
                base_section, overlay_data, overlay_type
            )
            ...
            return self._merge_generic(
                base_section, overlay_data, section_name, overlay_type
            )
            ...
            if field_name in overlay_data:
                ...
                if old_value != new_value and old_value is not None:
                    conflict = MergeConflict(
                        ...
                        project_value=(
                            new_value if overlay_type == OverlayType.PROJECT else None
                        ),
            ...
            for cmd_name, cmd_data in overlay_commands.items():
                if cmd_name in base_commands:
                    ...
                    conflict = MergeConflict(
                        ...
                        team_value=(
                            cmd_data if overlay_type == OverlayType.TEAM else None
                        ),
                        project_value=(
                            cmd_data if overlay_type == OverlayType.PROJECT else None
                        ),
                        resolution=ConflictResolution.PROJECT_WINS,
            ...
            if hasattr(base_obj, key):
                ...
                if old_value != value and old_value is not None:
                    conflict = MergeConflict(
                        ...
                        project_value=(
                            value if overlay_type == OverlayType.PROJECT else None
                        ),
                        resolution=ConflictResolution.PROJECT_WINS,
    ...
    def detect_conflicts(
        self, team_overlay: PersonaOverlay, project_overlay: PersonaOverlay
    ) -> List[MergeConflict]:
    ...
    def _get_modification_paths(
        self, modifications: Dict[str, Any], prefix: str = ""
    ) -> Dict[str, Any]:
    ...
    def _is_command_definition(self, value: Dict[str, Any]) -> bool:
        ...
        return isinstance(value, dict) and any(
            key in command_keys for key in value.keys()
        )
    ...
    def invalidate_cache(
        ...
        for key in self.cache.keys():
            parts = key.split(":")
            if len(parts) >= 3:
                key_persona_id, key_team_id, key_project_id = (
                    parts[0],
                    parts[1],
                    parts[2],
                )
                ...
                should_remove = (
                    (persona_id is None or key_persona_id == persona_id)
                    and (team_id is None or key_team_id == team_id)
                    and (project_id is None or key_project_id == project_id)
                )
