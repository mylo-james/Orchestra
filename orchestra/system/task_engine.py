...
class TaskExecutionError(Exception):
    ...
    def __init__(
        self, message: str, task_id: Optional[str] = None, step: Optional[str] = None
    ):
...
class TaskEngine:
    ...
    def __init__(self, execution_timeout: int = 300, max_retries: int = 3):
        ...
        logger.info(
            f"Initialized TaskEngine (timeout: {execution_timeout}s, max_retries: {max_retries})"
        )
    ...
    def execute_task(
        ...
        # Initialize result
        result = TaskExecutionResult(success=False, task_id=task_id, outputs={})
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                with self._timeout_context(self.execution_timeout):
                    execution_result = self._execute_task_steps(
                        steps, context, metadata
                    )
        ...
        if result.success:
            logger.info(
                f"Task execution completed: {task_id} (time: {result.execution_time:.3f}s, retries: {result.retry_count})"
            )
        else:
            logger.error(
                f"Task execution failed: {task_id} (time: {result.execution_time:.3f}s, retries: {result.retry_count})"
            )
    ...
    def _parse_task_steps(self, content: str) -> List[Dict[str, Any]]:
        ...
        sequential_match = re.search(
            r"##\s+SEQUENTIAL\s+Task\s+Execution.*?\n(.*?)(?=\n##|\n#|$)",
            content,
    ...
    def _execute_step(
        self, step: Dict[str, Any], context: Dict[str, Any], metadata: ResourceMetadata
    ) -> Dict[str, Any]:
        ...
        for instruction in instructions:
            try:
                ...
                if processed_instruction.startswith("Load "):
                    ...
                    result = self._execute_load_instruction(
                        processed_instruction, context
                    )
                    step_outputs.update(result)

                elif processed_instruction.startswith("If "):
                    # Conditional instruction
                    result = self._execute_conditional_instruction(
                        processed_instruction, context
                    )
                    step_outputs.update(result)
        ...
        return {"outputs": step_outputs, "warnings": step_warnings}
    ...
    def _execute_conditional_instruction(
        self, instruction: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        with self._execution_lock:
            if task_id in self._active_executions:
                ...
                if thread.is_alive():
                    ...
                    logger.warning(
                        f"Task cancellation requested for {task_id} (not fully implemented)"
                    )
                    return True
                else:
    ...
    def get_active_tasks(self) -> List[str]:
        ...
        with self._execution_lock:
            return [
                task_id
                for task_id, thread in self._active_executions.items()
                if thread.is_alive()
            ]
