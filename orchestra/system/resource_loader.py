...
class ResourceValidationError(Exception):
    ...
    def __init__(self, message: str, resource_id: Optional[str] = None, errors: Optional[List[str]] = None):
...
class ResourceLoader:
    ...
    def __init__(self, base_path: Path = Path(".bmad-core"), cache_enabled: bool = True, hot_reload_enabled: bool = False):
        """
    ...
    def discover_resources(self, resource_type: ResourceType) -> List[ResourceMetadata]:
        ...
        logger.info(f"Discovered {len(resources)} resources of type {resource_type.value}")
        return resources

    def load_resource(self, resource_id: str, resource_type: ResourceType) -> ResourceLoadResult:
        """
        ...
        try:
            ...
            logger.info(f"Successfully loaded resource: {resource_id} (load_time: {result.load_time:.3f}s)")
            return result
    ...
    def validate_resource(self, metadata: ResourceMetadata, content: str) -> ValidationResult:
        """
        ...
        logger.debug(f"Resource validation completed: {metadata.id} (valid: {is_valid})")
    ...
    def resolve_dependencies(self, metadata: ResourceMetadata) -> List[ResourceMetadata]:
        """
        ...
        for dep_id in metadata.dependencies:
            ...
            for resource_type in [ResourceType.TASK, ResourceType.TEMPLATE, ResourceType.CHECKLIST]:
                ...
                logger.warning(f"Could not resolve dependency: {dep_id} for resource: {metadata.id}")
        
        logger.debug(f"Resolved {len(resolved_dependencies)}/{len(metadata.dependencies)} dependencies for {metadata.id}")
        return resolved_dependencies

    def _extract_resource_metadata(self, file_path: Path, resource_type: ResourceType) -> ResourceMetadata:
        ...
        # Extract description
        description = ""
        desc_match = re.search(r'##\s+Purpose\s*\n\n(.+?)(?=\n##|\n#|$)', content, re.DOTALL)
        if not desc_match:
        ...
        dependencies = []
        dep_matches = re.findall(r'(?:depends?|requires?):\s*(.+)', content, re.IGNORECASE)
    ...
    def _load_resource_from_disk(self, resource_id: str, resource_type: ResourceType) -> ResourceLoadResult:
    ...
    def _get_from_cache(self, resource_id: str, resource_type: ResourceType) -> Optional[ResourceLoadResult]:
            ...
            if self.hot_reload_enabled and cached_result.metadata:
                ...
                if file_path.exists():
                    ...
                    if current_mtime > cached_mtime:
                        ...
                        logger.debug(f"File changed, invalidating cache for: {resource_id}")
    ...
    def _cache_resource(self, resource_id: str, resource_type: ResourceType, result: ResourceLoadResult):
        """Cache a resource result."""
        with self._cache_lock:
            cache_key = self._generate_cache_key(resource_id, resource_type, result.metadata.version)
    ...
    def _generate_cache_key(self, resource_id: str, resource_type: ResourceType, version: str) -> str:
    ...
    def _validate_template_content(self, content: str) -> List[str]:
        ...
        if not jinja2_vars and not bmad_vars:
            errors.append("Template should contain template variables ({variable} or {{variable}})")
