...
class ChecklistExecutionError(Exception):
    ...
    def __init__(self, message: str, checklist_id: Optional[str] = None, item_id: Optional[str] = None):
...
class ChecklistEngine:
    ...
    def __init__(self, interactive_mode: bool = False, auto_check_enabled: bool = False):
        """
        ...
        logger.info(f"Initialized ChecklistEngine (interactive: {interactive_mode}, auto_check: {auto_check_enabled})")

    def execute_checklist(self, metadata: ResourceMetadata, content: str, context: Dict[str, Any]) -> ChecklistExecutionResult:
        """
        ...
        try:
            ...
            if context.get("validate", True):
                result.validation_results = self._validate_checklist_completion(items, context)
            
            result.success = True
            
            logger.info(f"Checklist execution completed: {checklist_id} ({result.completion_percentage:.1f}% complete)")
    ...
    def _process_checklist_items(self, items: List[ChecklistItem], context: Dict[str, Any], result: ChecklistExecutionResult):
        ...
        for item in items:
            try:
                ...
                if self.auto_check_enabled and item.auto_check_rule and item.status == ChecklistItemStatus.PENDING:
                ...
                if item.validation_rule:
                    validation_result = self._evaluate_validation_rule(item.validation_rule, context)
                    if not validation_result:
                        result.warnings.append(f"Validation failed for item: {item.text}")
    ...
    def _calculate_completion_stats(self, result: ChecklistExecutionResult):
        ...
        if applicable_items > 0:
            result.completion_percentage = (result.completed_items / applicable_items) * 100.0
        else:
    ...
    def _validate_checklist_completion(self, items: List[ChecklistItem], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for item in items:
            if item.section and any(req in item.section for req in required_sections):
                if item.status not in [ChecklistItemStatus.COMPLETED, ChecklistItemStatus.NOT_APPLICABLE]:
                    validation_results["required_items_complete"] = False
                    validation_results["validation_errors"].append(f"Required item not complete: {item.text}")
        
        # Check context-based validations
        if context.get("test_status") == "failing":
            validation_results["validation_warnings"].append("Tests are failing - review required")
        
        if context.get("coverage", 0) < 90:
            validation_results["validation_warnings"].append("Code coverage below 90% - review required")
        
        validation_results["overall_valid"] = (
            validation_results["required_items_complete"] and
    ...
    def get_checklist_summary(self, result: ChecklistExecutionResult) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending_items": result.total_items - result.completed_items - result.not_applicable_items,
    ...
    def export_checklist_report(self, result: ChecklistExecutionResult, format: str = "markdown") -> str:
