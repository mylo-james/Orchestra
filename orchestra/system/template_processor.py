...
from jinja2 import Template, Environment, select_autoescape, TemplateError, UndefinedError
...
class TemplateRenderError(Exception):
    ...
    def __init__(self, message: str, template_id: Optional[str] = None, line_number: Optional[int] = None):
...
class TemplateProcessor:
    ...
    def __init__(self, template_engine: str = "jinja2", auto_escape: bool = True, sandbox_mode: bool = True):
        """
        ...
        logger.info(f"Initialized TemplateProcessor (engine: {template_engine}, auto_escape: {auto_escape}, sandbox: {sandbox_mode})")

    def render_template(self, metadata: ResourceMetadata, content: str, context: Dict[str, Any]) -> TemplateRenderResult:
        """
        ...
        try:
            ...
            variables_used, variables_missing = self._analyze_template_variables(template, context)
            ...
            result.rendered_content = self._postprocess_content(rendered_content, metadata)
            result.success = True
    ...
    def _postprocess_content(self, content: str, metadata: ResourceMetadata) -> str:
        ...
        processed = re.sub(r'\n\s*\n\s*\n', '\n\n', processed)  # Multiple blank lines to double
    ...
    def _analyze_template_variables(self, template: Template, context: Dict[str, Any]) -> tuple[List[str], List[str]]:
        """Analyze template variables usage."""
        try:
            # Get all variables referenced in template
            referenced_vars = self._extract_variables_from_ast(template.environment.parse(template.source))
