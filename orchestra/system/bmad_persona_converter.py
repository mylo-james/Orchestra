...
class PersonaValidationError(Exception):
    ...
    def __init__(self, message: str, persona_id: Optional[str] = None, field: Optional[str] = None):
...
class BmadPersonaConverter:
    ...
    def __init__(self, output_directory: Path = Path("orchestra/personas"), validation_enabled: bool = True):
        """
        ...
        logger.info(f"Initialized BMad persona converter with output directory: {output_directory}")

    def identify_target_personas(self, bmad_personas: List[BmadContentItem]) -> List[BmadContentItem]:
        """
        ...
        persona_items = [item for item in bmad_personas if item.content_type == BmadContentType.PERSONA]
    ...
    def convert_all_personas(self, target_personas: List[BmadContentItem]) -> List[PersonaConversionResult]:
        """
        ...
        logger.info(f"Batch conversion completed: {successful_count}/{len(target_personas)} successful")
        
        return results

    def convert_and_save_all(self, target_personas: List[BmadContentItem]) -> List[PersonaConversionResult]:
        """
    ...
    def validate_persona_schema(self, persona_spec: PersonaSpec) -> ValidationResult:
        ...
        try:
            ...
            if not persona_spec.version or not re.match(r'^\d+\.\d+(\.\d+)?$', persona_spec.version):
    ...
    def _extract_detailed_metadata(self, bmad_persona: BmadContentItem) -> Dict[str, any]:
        ...
        try:
            ...
            if bmad_persona.path.exists():
                ...
                raise FileNotFoundError(f"BMad persona file not found: {bmad_persona.path}")
    ...
    def _parse_bmad_content(self, content: str) -> Dict[str, any]:
        ...
        tools = re.findall(r'(?:tool|dependency|depends):\s*(.+)', content, re.IGNORECASE)
        if tools:
            metadata['tools'] = [tool.strip() for tool in tools]
        
        return metadata

    def _create_persona_spec(self, bmad_persona: BmadContentItem, metadata: Dict[str, any]) -> PersonaSpec:
        ...
        if 'domains' in metadata or 'expertise_areas' in metadata:
            knowledge_context = KnowledgeContext(
                ...
                memory_retention_policy=metadata.get('memory_retention_policy', 'session')
            )
